...
int main(){
    ...
    double timer;
        ...
        timer = omp_get_wtime();
        #if defined(_ANTIDIAGONAL)
            entries_visited = llcs_antidiagonal(X, Y, M);
        #elif defined(_TASKS)
            entries_visited = llcs_parallel_tasks(X, Y, M);
        #elif defined(_TASKSV2)
            entries_visited = llcs_parallel_tasks_v2(X, Y, M);
        #elif defined(_TASKSV3)
            entries_visited = llcs_parallel_tasks_v3(X, Y, M);
        #elif defined(_TASKLOOP)
            entries_visited = llcs_parallel_taskloop(X, Y, M);
        #elif defined(_TASKLOOPV2)
            entries_visited = llcs_parallel_taskloop_v2(X, Y, M);
        #elif defined(_TASKLOOPV3)
            entries_visited = llcs_parallel_taskloop_v3(X, Y, M);
        #endif
